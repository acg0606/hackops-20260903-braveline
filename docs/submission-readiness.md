...
- **HOLD — restore verification:** the previous UI could display restore success after an operation failure while retaining an active entitlement. A fresh device run must verify successful restore with corrected receipt handling.
...
- **HOLD — media:** the existing local asset is a 56-second English captioned screenshot montage from retained Android emulator evidence. Complete a submission-suitable demo, publish it to a public YouTube or Vimeo URL, and retain an anonymous readback. No public video is verified yet.
- **HOLD — Devpost:** recheck the logged-in registration, populate the entry, and submit only after every required receipt exists.

The APKs and screenshots are local emulator evidence, not a Play Store transaction, public store, eligibility, public-video, or submission receipt. The RevenueCat Test Store receipt records the simulated annual purchase and active entitlement; successful restore re-verification is pending. The UI-hardening APK is an x86_64 test build signed with the repository's current release signing configuration; it is not a Play Store artifact.
